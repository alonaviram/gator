...
const deleteFeedFollowForUserFeed = `-- name: DeleteFeedFollowForUserFeed :one
DELETE FROM feed_follows 
WHERE user_id=$1 AND feed_id=$2
RETURNING id, created_at, updated_at, user_id, feed_id
`

type DeleteFeedFollowForUserFeedParams struct {
	UserID uuid.UUID
	FeedID uuid.UUID
}

func (q *Queries) DeleteFeedFollowForUserFeed(ctx context.Context, arg DeleteFeedFollowForUserFeedParams) (FeedFollow, error) {
	row := q.db.QueryRowContext(ctx, deleteFeedFollowForUserFeed, arg.UserID, arg.FeedID)
	var i FeedFollow
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
